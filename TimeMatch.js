/*
---
План. Вывести текущее время.
0) по текущему дню квдртики по часам и темноте
1)текущие день ночь в биты и вывод
---
Опции добвить:
-источник времени
-текущее время менять
---
*/
var date = new Date();
...
// 86 400 000 = 60*60*24*1000
console.log('Время UTC '+new Date(now_utc-86400000)); 
console.log('Время UTC '+date.toISOString());
console.log('месяц день '+ new Date(now_utc).getMonth() +' '+ new Date(now_utc).getDay()  );  // 
console.log('Время UTC '+new Date(now_utc)); 
console.log('=======================')
console.dir(now_utc)
console.log('=======================')
console.dir(new Date(now_utc)	)
...
app.component('custom-form',{
  ...
  My {{ title }}
  <br>
  <input v-bind:label="emailLabel"  :modelValue="email"  type="email" />
  <br>
  <input type="password" v-model="password" label="Введи пароль"/>
  <button> Кнопка формы</button>
  </div>
  ...
  methods:{	
      ...
      handleSubmit(){
	    console.log('submited заполнено'+this.email, this.password)
	  }
  }
...
app.component('time-zone-list',{
  ...
  data(){
    return{
    myTimezone:-(new Date().getTimezoneOffset() / 60)
    }
  },
  created(){
    //myTimezone = new Date().toString() //it works if put directly to DATA section, but IMHO should be here
	}
...
myTimezone = new Date()
console.log(myTimezone.getTimezoneOffset()/60)
console.log(myTimezone.toString())

var offset = new Date().getTimezoneOffset();

console.log('rr'+offset);
